...
def main():
    ...
    CAR_test = DataLoader(CAR_test, batch_size=1, shuffle=True, num_workers=0)
    ...
    for epoch in range(Epochs):
        ...
        for batch_idx, (img, label) in enumerate(CAR_train):
            ...
            model.zero_grad()
            loss.backward()
            for idx in model.parameters():
                idx.data.sub_(idx.grad.data * lr)
        if epoch % 2 == 0:
            model.eval()
            total_correct = 0
            total_num = 0
            with torch.no_grad():
                for batch_idx, (img, label) in enumerate(CAR_test):
                    logits = model(img)
            ...
            if accuracy > accuracy_pre:
                ...
                fd.close()
                ################################
                save_path = os.path.join('./model', 'test_backup_' + str(epoch) + '.pth')
                torch.save(model.state_dict(), save_path)
    last_model = "./model/test_backup_last.pth"
